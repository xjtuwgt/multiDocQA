...
class PairwiseCEFocalLoss(nn.Module):
    ...
    def forward(self, scores: T, targets: T, target_len: T=None):
        ...
        loss = None
        pair_num = 0
        device_ = scores.device
        for idx in range(batch_size):
            pos_idx = (mask_flag[idx] >= 1).nonzero(as_tuple=False).squeeze() ## support sentence/doc label (0, 1, 2)
            neg_idx = (mask_flag[idx] == 0).nonzero(as_tuple=False).squeeze()
            if len(pos_idx.shape) == 0:
                pos_idx = torch.tensor([pos_idx], device=device_)
            if len(neg_idx.shape) == 0:
                neg_idx = torch.tensor([neg_idx], device=device_)
