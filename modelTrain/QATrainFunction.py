...
from hotpotEvaluation.hotpotEvaluationUtils import log_metrics, supp_doc_evaluation, answer_type_test, supp_sent_test, \
...
def train_step_qa(model, batch, optimizer, args):
    ...
    ans_type_loss, answer_span_loss, supp_doc_loss, supp_sent_loss = loss_res['answer_type_loss'], \
                                                                     loss_res['span_loss'], \
                                                                     loss_res['doc_loss'], loss_res['sent_loss']
    supp_doc_pair_loss = loss_res['doc_pair_loss']
    task_name = args.task_name
    if task_name == 'doc_sent_ans':
        train_loss = supp_doc_loss + supp_sent_loss + ans_type_loss + supp_doc_pair_loss * args.pair_score_weight + \
                     answer_span_loss * args.span_weight
    else:
        raise ValueError('Task name %s not supported' % task_name)
    ...
    log = {
        ...
        'ans_type_loss': ans_type_loss.mean().detach().item(),
        'ans_span_loss': answer_span_loss.mean().detach().item(),
        'sent_loss': supp_sent_loss.mean().detach().item(),
        'doc_loss': supp_doc_loss.mean().detach().item(),
        'doc_pair_loss': supp_doc_pair_loss.mean().detach().item()
    }
    return log
...
def validation_step_qa(model, batch, args):
    ...
    task_name = args.task_name
    if task_name == 'doc_sent_ans':
        valid_loss = supp_doc_loss + supp_sent_loss + ans_type_loss + supp_doc_pair_loss * args.pair_score_weight + \
                     answer_span_loss * args.span_weight
    else:
        raise ValueError('Task name %s not supported' % task_name)
    # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    output = {'valid_loss': valid_loss.mean().detach().item(), 'doc_metric': doc_metric_logs,
              'topk_metric': topk_metric_logs, 'valid_dict_output': valid_dictionary}
    # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    return output