...
def Hotpot_Train_Dev_Data_Preprocess(data: DataFrame, tokenizer: LongformerTokenizer):
    ...
    res_data = data[data_loader_res_columns]
    norm_encode_col_names = normalized_names + encode_names + ['_id']
    norm_encode_data = data[norm_encode_col_names]
    return data, res_data, norm_encode_data
...
def hotpotqa_preprocess_example():
    ...
    print('*' * 100)
    dev_data, _ = HOTPOT_DevData_Distractor(path=abs_hotpot_path)
    dev_data, dev_data_res, norm_dev_data_res = Hotpot_Train_Dev_Data_Preprocess(data=dev_data, tokenizer=longformer_tokenizer)
    print('Get {} dev records, encode records {} tokenized records {}'.format(dev_data.shape[0], dev_data_res.shape[0], norm_dev_data_res.shape[0]))
    dev_data.to_json(os.path.join(abs_distractor_wiki_path, 'hotpot_dev_distractor_wiki_all.json'))
    dev_data_res.to_json(os.path.join(abs_distractor_wiki_path, 'hotpot_dev_distractor_wiki_encoded.json'))
    norm_dev_data_res.to_json(os.path.join(abs_distractor_wiki_path, 'hotpot_dev_distractor_wiki_tokenized.json'))
    print('*' * 100)
    train_data, _ = HOTPOT_TrainData(path=abs_hotpot_path)
    train_data, train_data_res, norm_train_data_res = Hotpot_Train_Dev_Data_Preprocess(data=train_data, tokenizer=longformer_tokenizer)
    print('Get {} training records, encode records {} tokenized records {}'.format(train_data.shape[0], train_data_res.shape[0], norm_train_data_res.shape[0]))
    train_data.to_json(os.path.join(abs_distractor_wiki_path, 'hotpot_train_distractor_wiki_all.json'))
    train_data_res.to_json(os.path.join(abs_distractor_wiki_path, 'hotpot_train_distractor_wiki_encoded.json'))
    norm_train_data_res.to_json(os.path.join(abs_distractor_wiki_path, 'hotpot_train_distractor_wiki_tokenized.json'))
...
def gold_doc_hotpotqa_extraction_example():
    ...
    gold_dev_data, gold_dev_data_res, gold_norm_dev_data = Hotpot_Test_Data_PreProcess(data=gold_dev_data, tokenizer=longformer_tokenizer)
    print('Get {} dev-test records, encode records {}, normalized records {}'.format(gold_dev_data.shape[0], gold_dev_data_res.shape[0], gold_norm_dev_data.shape[0]))
    gold_dev_data.to_json(os.path.join(abs_distractor_wiki_path, 'hotpot_test_distractor_wiki_all_gold.json'))
    gold_dev_data_res.to_json(os.path.join(abs_distractor_wiki_path, 'hotpot_test_distractor_wiki_encoded_gold.json'))
    gold_norm_dev_data.to_json(os.path.join(abs_distractor_wiki_path, 'hotpot_test_distractor_wiki_tokenized_gold.json'))
    print('*' * 75)
