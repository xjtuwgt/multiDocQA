...
def HypeParameterSpace():
    ...
    batch_size = {'name': 'batch_size', 'type': 'fixed', 'value': 2}
    accu_grad_batch = {'name': 'accu_grad_size', 'type': 'choice', 'values': [4]}
    sent_threshold = {'name': 'sent_threshold', 'type': 'choice', 'values': [1.0]}
    task_name = {'name': 'task_name', 'type': 'choice', 'values': ['doc_sent_ans']} ##
    frozen_layer_num = {'name': 'frozen_layer', 'type': 'choice', 'values': [4]} #1, 2
    answer_span_weight = {'name': 'span_weight', 'type': 'choice', 'values': [0]}
    ...
    with_graph_training = {'name': 'with_graph_training', 'type': 'choice', 'values': [1]}# 0, 1
...
def generate_random_search_bash(task_num):
    ...
    for i in range(task_num):
        ...
        with open(bash_save_path + 'ir_run_' + task_id +'.sh', 'w') as rsh_i:
            command_i = 'bash irqarun.sh ' + parameter_id
