...
########################################################################################################################
########################################################################################################################
class LongformerQATensorizer(object):
    def __init__(self, tokenizer: LongformerTokenizer, max_length: int):
    ...
    def token_ids_to_tensor(self, token_ids):
        ...
        if len(token_ids) < seq_len:
            token_ids = token_ids + [self.get_pad_id()] * (seq_len - len(token_ids))
        if len(token_ids) > seq_len:
...
########################################################################################################################
def get_hotpotqa_longformer_tokenizer_with_addition_tokens(model_name=PRE_TAINED_LONFORMER_BASE, do_lower_case=True):
    tokenizer = LongformerTokenizer.from_pretrained(model_name, do_lower_case=do_lower_case)
    print(tokenizer.special_tokens_map)
    special_tokens_dict = {'additional_special_tokens': ['<q>', '</q>', '<t>', '</t>', '<p>']}
...
if __name__ == '__main__':
    ...
    tokenizer = get_hotpotqa_longformer_tokenizer(model_name=PRE_TAINED_LONFORMER_LARGE)
    print(tokenizer.special_tokens_map)
    print(len(tokenizer))
